...
if test -f $RELEASE_WORKFLOW; and grep -qF 'termux-repo:' $RELEASE_WORKFLOW; and grep -qF 'concurrency:' $RELEASE_WORKFLOW; and grep -qF 'timeout-minutes: 30' $RELEASE_WORKFLOW; and grep -qF 'termux-smoke-test:' $RELEASE_WORKFLOW; and grep -qF 'sudo timeout 10m bash scripts/termux-smoke.sh /tmp/termux-smoke /tmp/termux-smoke/*.deb /tmp/termux-root /tmp/termux-smoke/key.gpg' $RELEASE_WORKFLOW; and not grep -qF 'continue-on-error: true' $RELEASE_WORKFLOW
