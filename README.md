...
**YouTube OAuth re-auth (`linkari auth youtube --slot <name>`):** Runs a loopback OAuth flow on `--callback-addr` (default `127.0.0.1:53682`). On a laptop with a browser, this is the whole story. On a browserless cloud host (SSH/SSM session, no port forwarding), run the command from an interactive TTY: it still prints the authorization URL - open it in a browser on any machine, approve, and when the browser fails to load the redirect (expected, nothing is listening on that host's port from the browser's point of view), paste the full redirect URL or just the `code` value back into the terminal. The loopback callback and the pasted code race; whichever arrives first wins and the other is abandoned. Bare-code pastes skip state verification (state is enforced when a full URL is pasted); after 3 unparseable pastes the command exits with an error. Non-interactive stdin (scripts, CI, non-pty SSH) never arms the paste prompt - behavior there is unchanged. If the callback port is already bound and stdin is a TTY, the command degrades to paste-only mode with a warning instead of failing.
...
**Image text extraction pre-pass (EPIC-122):** When `image_text_extraction_enabled = true` (default: `false`), a Claude Haiku vision call runs before scoring for all `type=image` shares that pass the noise gate. The call transcribes all visible text verbatim (`model: claude-haiku-4-5`, 30s timeout, `max_concurrency=1`). When extracted text exceeds `image_short_circuit_bypass_min_chars` (default: `20`), the camera-photo short-circuit is suppressed and the transcript is included in the scoring prompt - 9 fields instead of 5 (`calling_package`, `relative_path`, `is_screenshot`, `url`, `transcribed_text` appended when present). Transcript files are persisted to `transcripts_dir` at `YYYYMMDD_<rowID>_IMG_<slug>.md` (default: `~/docs/transcripts`). CLI failure or timeout falls through to current scoring behavior; the share is never blocked. Feature is disabled by default for safe rollout.
