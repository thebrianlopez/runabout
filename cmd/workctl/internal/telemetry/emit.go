...
// Instrument wraps the root command's persistent pre-run hook to capture start
// time and the resolved subcommand. It handles both PersistentPreRunE and
// PersistentPreRun variants. Call Emit after rootCmd.Execute completes.
func Instrument(rootCmd *cobra.Command, cliName string) *Tracker {
	t := &Tracker{cliName: cliName, start: time.Now(), root: rootCmd}

	if existing := rootCmd.PersistentPreRunE; existing != nil {
		rootCmd.PersistentPreRunE = func(cmd *cobra.Command, args []string) error {
			t.start = time.Now()
			t.cmd = cmd
			return existing(cmd, args)
		}
	} else if existing := rootCmd.PersistentPreRun; existing != nil {
...
func (t *Tracker) Emit(cmdErr error) {
	cmd := t.cmd
	if cmd == nil && t.root != nil {
		// Subcommand overrode PersistentPreRunE — resolve from args.
		cmd, _, _ = t.root.Find(os.Args[1:])
	}
	if cmd == nil {
		return
	}
	...
	// Use CommandPath for full nested path (e.g. "workctl cache stats"),
	// then strip the CLI name prefix since buildEvent prepends it.
	cmdPath := cmd.CommandPath()
	subcmd := strings.TrimPrefix(cmdPath, t.cliName+" ")
