...
// scoreURLAsync runs the full server-side scoring pipeline for a uinit_* share.
...
//
// Pipeline: unsupported-check → Jina fetch (30s) → truncate → loadProfileTemplate
// → eval.Evaluate (Haiku) → ScoreByURL → archive → EnqueueDigestIfDue (FCM push).
//
